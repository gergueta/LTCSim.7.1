...
uses
  ...
  Controls, Db, ComCtrls, Forms, Dialogs, StdCtrls, Buttons, ExtCtrls,
  ...
  System.ImageList,
  LMDBaseMRUList, LMDStorBase, LMDCustomComponent,
  ...
  LMDCustomButton,
  LMDContainerComponent, LMDBaseDialog, LMDAboutDlg, LMDShBase, LMDStarter,
  LMDBrowseDlg, LMDCustomPanelFill,
  LMDButtonPanel, LMDCustomToolBar, LMDToolBar, Vcl.ActnCtrls, Vcl.ActnMan,
  Vcl.ActnMenus, LMDDockSpeedButton, LMDBaseControl, LMDBaseGraphicControl,
  LMDBaseGraphicButton, LMDCustomSpeedButton, LMDSpeedButton,
  LMDCustomFormComboBox, LMDFormComboBox, LMDCustomColorComboBox,
  LMDColorComboBox, System.Actions, Vcl.ActnList,
  LMDCustomScrollBox, LMDScrollBox, LMDShController,
...
type
  TMainForm = class(TForm)
    ...
    LMDLabeledCheckListBox1: TLMDLabeledCheckListBox;
    ...
    procedure ipwPingError(Sender: TObject; ErrorCode: Integer;
...
uses
  ProcessDialog, Rename, StimDlg, UpdateWarning,
  newRev, SchemToAscii,
  AscciToSchem, SymToAsy, AsyToSymbols, DataModule,
  NewRevWithoutProcessCode, LTCSimArchive, UpdateSchematics,
  DisplayHideAttr;
...
begin { TMainForm.runVerilogNetlist }
...
var
  sSchematicsFile, sFileExtension: String;
begin
  CodeSite.EnterMethod(Self, 'ActionNavigateSchematicsExecute');

  sFileExtension := '.sch';
  sSchematicsFile := cbSchematics.Text + sFileExtension;

  CodeSite.ExitMethod(Self, 'ActionNavigateSchematicsExecute');
...
begin
  CodeSite.EnterMethod(Self, 'ProjectNewExecute');
  ...
  CodeSite.ExitMethod(Self, 'ProjectNewExecute');
end;
...
var
  sSchTempFile: string;
  sCommand, sArgument, sWorkingDir: string;
begin { TMainForm.runSCSSchEditor }
...
Var
  ...
  I: Integer;
begin
  CodeSite.EnterMethod(Self, 'ReadXMLFile');
...
begin
  CodeSite.EnterMethod(Self, 'SaveXMLFile');
...
begin
  ...
  begin
    case tag of
      1:
        begin
          JamShellListMain.Filter := '*.sch';
          JamShellListMain.Update
        end;
      2:
        begin
          JamShellListMain.Filter := '*.asc';
          JamShellListMain.Update
        end;
      3:
        begin
          JamShellListMain.Filter := '*.sym';
          JamShellListMain.Update
        end;
      4:
        begin
          JamShellListMain.Filter := '*.spi';
          JamShellListMain.Update
        end;
      5:
        begin
          JamShellListMain.Filter := '*.net';
          JamShellListMain.Update
        end;
      6:
        begin
          JamShellListMain.Filter := '*.lvs';
          JamShellListMain.Update
        end;
      7:
        begin
          JamShellListMain.Filter := '*.apt';
          JamShellListMain.Update
        end;
      8:
        begin
          JamShellListMain.Filter := '*.v';
          JamShellListMain.Update
        end;
      9:
        begin
          JamShellListMain.Filter := '*.edf';
          JamShellListMain.Update
        end;
      10:
        begin
          JamShellListMain.Filter := '*.cir';
          JamShellListMain.Update
        end;
      11:
        begin
          JamShellListMain.Filter := '*.sp';
          JamShellListMain.Update
        end;
      12:
        begin
          JamShellListMain.Filter := '*.lvh';
          JamShellListMain.Update
        end;
      13:
        begin
          JamShellListMain.Filter := '*.alvh';
          JamShellListMain.Update
        end;
      14:
        begin
          JamShellListMain.Filter := '*.netz';
          JamShellListMain.Update
        end;
      20:
        begin
          JamShellListMain.Filter := '*.*';
          JamShellListMain.Update
        end;
    end
  end;
...
begin { TMainForm.CreateLTspiceIniFile }
...
procedure TMainForm.ipwPingError(Sender: TObject; ErrorCode: Integer;
  const Description: string);
begin
...
begin { TMainForm.FileExit }
  CodeSite.EnterMethod(Self, 'ExitLTCSimExecute');

  StopSCSShell;
  SaveLTCSimOptions(LTCSim.xmlFileName);
  Application.Terminate;

  CodeSite.ExitMethod(Self, 'ExitLTCSimExecute');
end; { TMainForm.FileExit }
...
var
  sSchemDirectory, sSchemFileName, sSchemZipDirectory, sDate, sTime: string;
  sCommand, sArgument, sWorkingDir, sZipFileName: string;
  sTreFile: string;
  // archiver: TZipForge;
begin
...

  {
    ...
    DateTimeToString(sDate, 'mmddyy', Now);
    DateTimeToString(sTime, 'hhnn', Now);
    //
    //  sZipFileName := IncludeTrailingPathDelimiter( sSchemDirectory )
    ...
    begin
    FileName := sZipFileName;
    // SFXStub := IncludeTrailingPathDelimiter(LTCSim.dir) + 'SFXStub.exe';
    OpenArchive(fmCreate);
    BaseDir := sSchemDirectory;
    ...
    begin
    MessageDlg('File ' + sTreFile +
    ' not found. Open with Navigator and save it first!', mtError, [mbOK], 0);
    end
  }
...
begin
  CodeSite.EnterMethod(Self, 'ProjectInitialize2Execute');
  ...
  CodeSite.ExitMethod(Self, 'ProjectInitialize2Execute');
end;
...
begin
  ...
  CodeSite.ExitMethod(Self, 'ActionProjectNewRev');
end;
...
begin
  CodeSite.EnterMethod(Self, 'ProjectSave2Execute');
  ...
  CodeSite.ExitMethod(Self, 'ProjectSave2Execute');
end;
...
var
  ...
  tempToolInUse: TTool;
begin
  CodeSite.EnterMethod(Self, 'ProjectSchematicsToAPTExecute');
  ...
  begin
    ...
    begin
      ...
      if DirectoryExists(LTCSim.localProjectsDir) then
      begin
        ...
        sTemp := IncludeTrailingPathDelimiter(LTCSim.localProjectsDir) +
        ...
        MessageDlg('Directory ' + LTCSim.localProjectsDir + ' not found!',
          mtError, [mbOK], 0);
      Screen.Cursor := crDefault;
    end;
  end
  ...
  CodeSite.ExitMethod(Self, 'ProjectSchematicsToAPTExecute');
end;

procedure TMainForm.ActionSchematicsSnapshotExecute(Sender: TObject);
begin
  CodeSite.EnterMethod(Self, 'SchematicsSnapshotExecute');
  ...
  if OpenDialog.Execute then
  begin
    LTCSimArchiveSnapshot(ChangeFileExt(OpenDialog.FileName, '.sch'));
  end;
...
var
  ...
  // archiver: TZipForge;
begin
  CodeSite.EnterMethod(Self, 'SnapshotViewerExecute');
