...
static int Do_Symbol_Instance( SI_PTR si, struct _inst* inst )
{
  ...
  if ((( bCMOS ) || ( bBipolar ) || ( bResistor )) && !( bTitle ))
	...
	{
		char buff[256];
		sprintf( buff, "Get_SIA( %s, %d ): '%s'",
			Get_SIA( si, NAME ), WIDTH, val );
		MessageBox(NULL,buff,"LtcSimSchShrink",MB_OK);
	  strcpy( OldAttValue, val );
/*
	  if ( Tcl_GetDouble( interp, OldAttValue, dValue ) == TCL_OK )
	    {
...
*/
	  if(1){if(1){
			sprintf( NewAttValue, "%.3f", atol(val) * 2.0/3.0 );
			Add_IA( si, WIDTH, NewAttValue );
			sprintf( buff, "Add_IA( %s, %d ) did: '",
				Get_SIA( si, NAME ), WIDTH );
			strcat( buff, Get_SIA( si, WIDTH ));
			strcat( buff, "'" );
			MessageBox(NULL,buff,"LtcSimSchShrink",MB_OK);
...
{
  MessageBox(NULL,"Grab me in the debugger!","LtcSimSchShrink",MB_OK);
  ...
  if ( LoadSchematic( schFileName )) {
    ...
    char msg[300];
    sprintf( msg, "Unable to process schematic %s", schFileName );
    MajorError( msg );
  }
  return( 1 );
}
